 ...
 * https://github.com/nikolasdogan/simple-file-manager
 Kısmı Olarak Alıntıdır Bazı Kısımları Bu Kod Benzeri Varıdı Deyip Gömmeyin Sonra :P
 */
...
$default_timezone = 'Europe/Turkey'; // UTC+3
...
<th>Adı</th><th style="width:10%">Türü/Boyutu</th>
...
if ($parent !== false) {
    ?>
<tr><td></td><td colspan="<?php echo !FM_IS_WIN ? '6' : '4' ?>"><i class=""></i></a></td></tr>
<?php
}
foreach ($folders as $f) {
...
<?php endif; ?>
<td>
<a title="Direk Bağlantı" href="<?php echo fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f) ?>" target="_blank"><i class="icon-chain"></i></a>
<a title="İndir" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;dl=<?php echo urlencode($f) ?>"><i class="icon-download"></i></a>
</td></tr>
    <?php
    flush();
}
...
Dosya Boyutu: <b><span title="<?php printf('%s bytes', $all_files_size) ?>"><?php echo fm_get_filesize($all_files_size) ?></b></span>,
Dosyalar: <b><?php echo $num_files ?></b>,
Klasörler: <b><?php echo $num_folders ?></b>
</td></tr>
<?php
}
?>
